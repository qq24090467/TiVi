...
import it.neokree.materialtabs.at.markushi.ui.RevealColorView;
...
public class MaterialTab implements View.OnTouchListener {
	...
	public MaterialTab(Context ctx, boolean hasIcon) {
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			if (!hasIcon) {
				...
				Typeface typeFace = CustomFontsLoader.getTypeface(ctx, 0);
				if (typeFace != null) {
					text.setTypeface(typeFace);
				} else {
					System.out.println("fhksdhfkdshfkhsk");
				}
	...
	public void disableTab() {
		...
		if (text != null)
			this.text
					.setTextColor(Color.argb(0x99, Color.red(textColor), Color.green(textColor), Color.blue(textColor)));
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			if (!deviceHaveRippleSupport()) {
				// completeView.setBackgroundColor(Color.argb(0x80,
				// Color.red(accentColor), Color.green(accentColor),
				// Color.blue(accentColor)));
				completeView.setBackgroundColor(Color.argb(0x80, Color.red(primaryColor), Color.green(primaryColor),
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			...
			if (!deviceHaveRippleSupport()) {
				...
				// this.background.reveal(lastTouchedPoint.x,
				// lastTouchedPoint.y,
				// Color.argb(0x80, Color.red(accentColor),
				// Color.green(accentColor), Color.blue(accentColor)), 0,

				this.background.reveal(lastTouchedPoint.x, lastTouchedPoint.y,
						Color.argb(0x80, Color.red(primaryColor), Color.green(primaryColor), Color.blue(primaryColor)),
						0,
			...
			if (listener != null) {
				...
				if (active) {
					// if the tab is active when the user click on it it will be
					// reselect
