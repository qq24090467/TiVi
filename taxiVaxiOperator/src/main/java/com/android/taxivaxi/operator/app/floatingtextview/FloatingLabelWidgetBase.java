package com.android.taxivaxi.operator.app.floatingtextview;
...
public abstract class FloatingLabelWidgetBase<InputWidgetT extends View> extends FrameLayout {
	...
	@SuppressLint("NewApi") private void layoutChild(View child, int parentLeft, int parentTop, int parentRight, int parentBottom) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (savedState != null) {
			...
			// Because View#onRestoreInstanceState is protected, we got to ask
			// subclasses to do it for us
	...
	/**
	 * Give the opportunity to child classes to restore additional state
	 * variables they had saved
	 * 
	 * @param savedState
	 ...
	 * 
	 * @param inputWidgetState
	 *            The state of the input widget
	 */
	protected void restoreInputWidgetState(Parcelable inputWidgetState) {
	}

	/**
	 * Save the input widget state. Usually you will simply call the widget's
	...
	/**
	 * The default animator to use for the label. That method is called in init
	 * so that subclasses can provide their own specialized animator if
	 * appropriate.
	 * 
	 * @return
	 */
	...
	/**
	 * Specifies the layout to be inflated for the widget content. That layout
	 * must at least declare an EditText with id "flw_floating_label" and an
	 * input widget of the proper class with id "flw_input_widget"
	 * 
	...
	/**
	 * Initialise the widget: read attributes, inflate layout and set the basic
	 * properties
	 * 
	 * @param context
	 * @param attrs
	...
	protected void init(Context context, AttributeSet attrs, int defStyle) {
		...
		if (attrs == null) {
			...
			final TypedArray a = context
					.obtainStyledAttributes(attrs, R.styleable.FloatingLabelWidgetBase, defStyle, 0);
			...
			floatLabelTextSize = a.getDimension(R.styleable.FloatingLabelWidgetBase_flw_labelTextSize, getResources()
					.getDimensionPixelSize(R.dimen.flw_defaultLabelTextSize));
