...
/**
 * A default animator to move our label up when asked to float. This uses a
 * scale and alpha transformation too when floating.
 * <p/>
...
public class DefaultLabelAnimator<InputWidgetT extends View> implements LabelAnimator<InputWidgetT> {
	...
	private void setLabelAnchored(InputWidgetT inputWidget, View label, boolean isAnchored, boolean animateLabel) {
		...
		if (animateLabel) {
			label.setPivotX(0);
			label.setPivotY(0);
			label.animate().alpha(targetAlpha).x(targetX).y(targetY).scaleX(targetScale).scaleY(targetScale);
		} else {
		...
		Log.d("DefaultLabelAnimator", String.format(
				"Setting label state to (x=%.0f, y=%.0f, alpha=%.1f, scale=%.1f, anchored=%d)", targetX, targetY,
				targetAlpha, targetScale, isAnchored ? 1 : 0));
	}
